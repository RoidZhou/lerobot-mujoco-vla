...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a PI0 policy on the real UR5 + RG2 ZMQ nodes."
    )
    ...
    parser.add_argument(
        "--vlm-model-name",
        default=None,
        help="Optional PI0 tokenizer/VLM name or local PaliGemma path. Defaults to the checkpoint config.",
    )
    ...
    parser.add_argument(
        "--effort-key",
        default=None,
        help="Optional override for PI0 force input key, for example observation.force_torque.",
    )
    parser.add_argument(
        "--force-vqvae-ckpt",
        default=None,
        help="Optional override for a force_vqvae tokenizer checkpoint saved in the PI0 config.",
    )
    parser.add_argument(
        "--force-source",
...
class RealUR5PI0Infer:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        config = load_pi0_config(self.policy_path, self.device, self.vlm_model_name)
        if args.effort_key:
            config.effort_key = args.effort_key
        if args.force_vqvae_ckpt:
            config.force_vqvae_ckpt = args.force_vqvae_ckpt
        ...
        if OBS_STATE in non_force_state_keys:
            ...
            raise ValueError("PI0 policy has no robot state input feature.")
        ...
        if not action_features or int(action_features[0].shape[0]) < 7:
            raise ValueError("Expected a PI0 action feature with at least 7 dimensions.")
    ...
    def _read_predicted_force(self) -> np.ndarray | None:
        ...
        return predicted[0].numpy().astype(np.float32).reshape(-1)
...
def main() -> None:
    ...
    try:
        ...
        print("Starting PI0 real UR5 inference. Press Ctrl-C to stop.")
        step = 0
        while args.max_steps <= 0 or step < args.max_steps:
            ...
            if step % max(1, args.print_every) == 0:
                ...
                print(
                    ...
                    f"pred_force={np.array2string(infer.last_predicted_force, precision=3) if infer.last_predicted_force is not None else 'none'}"
                )
