...
class SimpleEnv2:
    ...
    def reset(self, seed = None):
        ...
        nut_init_pose, bolt_init_pose = self.get_obj_pose()
        self.obj_init_pose = np.concatenate([nut_init_pose, bolt_init_pose], dtype=np.float32)
        for _ in range(100):
            self.step_env()
    ...
    def set_instruction(self, given = None):
        ...
        #     self.instruction = f'Place the {obj1} mug, and put it on the plate.'
        #     if obj1 == 'red':
        ...
        self.instruction = given or 'Insert the bolt into the nut.'
        self.obj_target = 'bolt_body'
    ...
    def step(self, action):
        ...
        # gripper_cmd[[1,3]] *= 0.8
        # gripper_cmd = np.array([action[-1]]*1)* 1
        gripper_closed = 0.3
        gripper_open = 1.0
        gripper_cmd = np.array(
            [gripper_open if action[-1] > 0.5 else gripper_closed],
            dtype=np.float32,
        )
    ...
    def render(self, teleop=False, idx = 0):
        ...
        if teleop:
            ...
            align_pos, align_rpy, align_rel = self.get_alignment_debug_lines()
            self.env.viewer_text_overlay(text1='Align Pos', text2=align_pos)
            self.env.viewer_text_overlay(text1='Align RPY', text2=align_rpy)
            self.env.viewer_text_overlay(text1='Relative RPY', text2=align_rel)
            self.env.viewer_text_overlay(text1='Contact Force', text2=self.get_contact_force_debug())
    ...
    def get_alignment_debug(self):
        return ' | '.join(self.get_alignment_debug_lines())

    def get_alignment_debug_lines(self):
        p_nut, R_nut = self.env.get_pR_body('nut_body')
        p_bolt, R_bolt = self.env.get_pR_body('bolt_body')
        p_tcp, R_tcp = self.env.get_pR_body('tcp_link')
        tcp_delta = p_tcp - p_nut
        nut_delta = p_nut - p_bolt
        tcp_rpy = np.rad2deg(r2rpy(R_tcp))
        bolt_rpy = np.rad2deg(r2rpy(R_bolt))
        nut_rpy = np.rad2deg(r2rpy(R_nut))
        tcp_nut_rpy = np.rad2deg(r2rpy(R_nut.T @ R_tcp))
        nut_bolt_rpy = np.rad2deg(r2rpy(R_bolt.T @ R_nut))
        return (
            f"tcp-nut [{tcp_delta[0]:+.4f}, {tcp_delta[1]:+.4f}, {tcp_delta[2]:+.4f}] "
            f"nut-bolt [{nut_delta[0]:+.4f}, {nut_delta[1]:+.4f}, {nut_delta[2]:+.4f}]",
            f"tcp [{tcp_rpy[0]:+.1f}, {tcp_rpy[1]:+.1f}, {tcp_rpy[2]:+.1f}] "
            f"bolt [{bolt_rpy[0]:+.1f}, {bolt_rpy[1]:+.1f}, {bolt_rpy[2]:+.1f}] "
            f"nut [{nut_rpy[0]:+.1f}, {nut_rpy[1]:+.1f}, {nut_rpy[2]:+.1f}]",
            f"tcp-nut [{tcp_nut_rpy[0]:+.1f}, {tcp_nut_rpy[1]:+.1f}, {tcp_nut_rpy[2]:+.1f}] "
            f"nut-bolt [{nut_bolt_rpy[0]:+.1f}, {nut_bolt_rpy[1]:+.1f}, {nut_bolt_rpy[2]:+.1f}]",
        )

    def get_contact_force_debug(self):
        nut_bolt_forces = []
        gripper_bolt_forces = []

        for contact_idx in range(self.env.data.ncon):
            contact = self.env.data.contact[contact_idx]
            body1 = self.env.body_names[self.env.model.geom_bodyid[contact.geom1]]
            body2 = self.env.body_names[self.env.model.geom_bodyid[contact.geom2]]
            bodies = {body1, body2}

            force_local = np.zeros(6, dtype=np.float64)
            mujoco.mj_contactForce(self.env.model, self.env.data, contact_idx, force_local)
            force_norm = np.linalg.norm(force_local[:3])

            if bodies == {'nut_body', 'bolt_body'}:
                nut_bolt_forces.append(force_norm)
            elif 'bolt_body' in bodies and any(body.startswith('rg2_gripper') for body in bodies):
                gripper_bolt_forces.append(force_norm)

        def summarize(forces):
            if not forces:
                return 'n=0 sum=0.00 max=0.00'
            forces = np.asarray(forces)
            return f'n={len(forces)} sum={forces.sum():.2f} max={forces.max():.2f}'

        return (
            f'nut-bolt {summarize(nut_bolt_forces)} | '
            f'gripper-bolt {summarize(gripper_bolt_forces)}'
        )
    ...
    def get_joint_state(self):
        ...
        gripper = self.env.get_qpos_joint('gripper_finger2_joint')
    ...
    def teleop_robot(self):
        ...
        Keys:
            ...
            T: screw downward while rotating
            G: screw upward while rotating back
            ---------   
        ...
        move_step = 0.0005
        rot_step = 0.015
        screw_pitch = 0.0025  # M20 thread pitch: 2.5 mm per revolution.
        screw_rot_step = 0.02
        screw_linear_step = screw_pitch * screw_rot_step / (2 * np.pi)
        dpos = np.zeros(3)
        drot = np.eye(3)
        ...
        # T：一边向下，一边旋转，模拟拧入
        # G：
        if self.env.is_key_pressed_repeat(key=glfw.KEY_T):
            dpos += np.array([0.0, 0.0, -screw_linear_step])
            drot = rotation_matrix(angle=-screw_rot_step, direction=[0.0, 1.0, 0.0])[:3, :3]
        if self.env.is_key_pressed_repeat(key=glfw.KEY_G):
            dpos += np.array([0.0, 0.0, screw_linear_step])
            drot = rotation_matrix(angle=screw_rot_step, direction=[0.0, 1.0, 0.0])[:3, :3]
    ...
    def check_success(self):
        '''
        Manually mark the current episode as successful.
        Press ENTER to save the episode and reset the environment.
        '''
    ...
    def get_obj_pose(self):
        ...
        '''
        p_nut = self.env.get_p_body('nut_body')
        p_bolt = self.env.get_p_body('bolt_body')
        return p_nut, p_bolt
    
    # 记录初始位置
    def set_obj_pose(self, p_nut, p_bolt):
        '''
        ...
        self.env.set_p_base_body(body_name='nut_body', p=p_nut)
        self.env.set_R_base_body(body_name='nut_body', R=np.eye(3, 3))

        self.env.set_p_base_body(body_name='bolt_body', p=p_bolt)
        self.env.set_R_base_body(body_name='bolt_body', R=rpy2r(np.deg2rad([180, 0, 0])))
