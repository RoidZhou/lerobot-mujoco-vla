...
class SimpleEnv:
    ...
    def reset(self, seed = None):
        ...
        # 改成np.array([0.0]*2)，因为ur5的gripper只有两个关节，而不是四个。
    ...
    def step(self, action):
        ...
        # 改成*2，因为ur5的gripper只有两个关节
        gripper_cmd = np.array([action[-1]]*2)* 0.5
        # gripper_cmd[[1,3]] *= 0.8
    ...
    def get_joint_state(self):
        ...
        gripper = self.env.get_qpos_joint('rg2_finger_joint1')
    ...
    def check_success(self):
        ...
        if np.linalg.norm(p_mug[:2] - p_plate[:2]) < 0.1 and np.linalg.norm(p_mug[2] - p_plate[2]) < 0.6 and self.env.get_qpos_joint('rg2_finger_joint1') > 0.5:
