...
# 在139行加了个调试print，输出gripper的输入值和最终的gripper_cmd值，看看是否正确传递了gripper的状态。
# action的最后一个元素应该是gripper的状态（0或1），gripper_cmd应该是一个长度为4的数组，表示四个gripper关节的命令值。
# gripper_cmd应该是[0, 0, 0, 0]（表示打开）或者[1.0, 0.8, 1.0, 0.8]（表示关闭）。
...
class SimpleEnv:
    ...
    def reset(self, seed = None):
        ...
        # 改成np.array([0.0]*1)，因为ur5的gripper只有1个控制器。
        # 原来：self.q = np.concatenate([q_zero, np.array([0.0]*4)])
    ...
    def step(self, action):
        ...
        # 0 is closed and 1 is open for this gripper. The bolt is small, so do
        # not command a fully closed target; otherwise the fingers can squeeze
        # through or push the bolt away.
        gripper_closed = 0.2
        gripper_open = 1.0
        ...
        # print(f"gripper input: {action[-1]:.2f}, cmd: {gripper_cmd}")  # 加这行，调试
    ...
    def get_joint_state(self):
        ...
        # 所有的rg2_finger_joint1都改成gripper_finger2_joint
    ...
    def set_obj_pose(self, p_nut, p_bolt):
        ...
        self.env.set_R_base_body(body_name='bolt_body', R=np.eye(3, 3))
