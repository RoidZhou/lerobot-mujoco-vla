...
# 新增了真实的相机接口，camera1和camera2分别对应第三视角和手腕视角
# 新增URrobot参数，在deploy中会用URRobot实例化
class SimpleEnv2:
    def __init__(self, 
                 xml_path,
        ...
        self.URrobot = URrobot
        self.init_joint_state = np.array([1.396263,-1.047198, 0.698132, -1.256637, -1.483530, 1.396263])
    ...
    def reset(self, seed = None):
        ...
        '''
        self.URrobot.command_joint_pose(self.init_joint_state)
        self.q = np.concatenate([self.init_joint_state, np.array([0.0]*1)])
    ...
    # 要改这里！！！
    def step_env(self):
        # 如果连接了真实的机器人，就发送命令到机器人
        if self.URrobot is not None:
            self.URrobot.command_joint_pose(joint_state=self.q)
        
        # self.env.step(self.q) # 这是仿真的


    # get_fixed_cam_rgb是从mujoco中获得图像的函数，如果用真实相机的话不用这个函数，直接在grab_image里面调用相机接口获取图像就行了
    # 原来只用了两个相机rgb_agent和rgb_ego，现在改成从两个真实相机获取图像，在init里面初始化相机，grab_image里面获取图像
    def grab_image(self):
        '''
        grab images from the environment
        returns:
            rgb_agent: np.array, rgb image from the agent's view 第三视角
            rgb_ego: np.array, rgb image from the egocentric 手腕视角
                rgb_top: np.array, rgb image from the top view 俯视图 但是原来注释掉了
                rgb_side: np.array, rgb image from the side view 侧视图 没有被注释 但是原来return里也没有这个
        '''
        if self.camera1 is None or self.camera2 is None:
            raise ValueError("Camera not initialized!")
        
        rgb_agent = self.camera1.read()
        rgb_ego   = self.camera2.read()
        # 确保是 numpy 数组格式
        if not isinstance(rgb_agent, np.ndarray):
            rgb_agent = np.array(rgb_agent)
        if not isinstance(rgb_ego, np.ndarray):
            rgb_ego = np.array(rgb_ego)
    ...
    def get_joint_state(self):
        ...
        '''
        # 连接了真实的机器人，从机器人那里获取关节状态
        if self.URrobot is not None:
            qpos = self.URrobot.get_joint_state()
        gripper_pose = self.URrobot.last_gripper_pose
        gripper_cmd = 1.0 if gripper_pose > 50 else 0.0
    ...
    def check_success(self):
        ...
        # 判断成功的条件是：杯子在盘子上方0.1范围内，并且高度差小于0.6，同时夹爪处于打开状态，并且末端执行器高度大于0.9
        if np.linalg.norm(p_mug[:2] - p_plate[:2]) < 0.1 and np.linalg.norm(p_mug[2] - p_plate[2]) < 0.6 and self.env.get_qpos_joint('gripper_finger2_joint') > 0.3:
